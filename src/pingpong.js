  ...
  PingPong.GameScene.prototype = {
    ...
    loadModels: function () {
      ...
      function onTableLoad(geometry, materials) {
        ...
        if (me.settings.table.texture) {
          tableSettings.map = THREE.ImageUtils.loadTexture(
            me.settings.table.texture
          );
        } else {
    ...
    checkBallHit: function () {
      ...
      if (
        ...
        paddle.position.z > ball.position.z
      ) {
        //store trayectory
        var trayectory = {
          time: Date.now(),
